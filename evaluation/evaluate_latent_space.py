...
def get_args():
    ...
    parser.add_argument("--output_dir", type=str, default="outputs/analysis", help="Directory to save plots and results.")
    ...
    parser.add_argument("--label_col", type=str, default=None, help="Optional: column with ground truth labels for ARI/NMI.")
...
def main():
    ...
    # Optional: ARI/NMI if ground truth labels are provided
    if args.label_col and args.label_col in df.columns:
        print("[INFO] Calculating ARI and NMI...")
        ari = adjusted_rand_score(df[args.label_col], df["Cluster"])
        nmi = normalized_mutual_info_score(df[args.label_col], df["Cluster"])
    ...
    metrics_df.to_csv(f"{args.output_dir}/metrics_report.csv", index=False)
